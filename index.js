    ...
    compiler.plugin('emit', (compilation, callback) => {
      ...
      if (_this.options && Object.keys(_this.options).length && !_this.options.product) {
        ...
        array.forEach(item => {
          ...
          <script>          
            window.wmlogConfig = {
              'product': ${JSON.stringify(_this.options.product)},
              'page': ${JSON.stringify(item)},
              'speed': ${_this.options.speed ? JSON.stringify(_this.options.speed) || {"sample":1,"auto":true,"ext":[]}},
              'exception': ${_this.options.exception ? JSON.stringify(_this.options.exception) || {"sample":1}}
            }
          `
